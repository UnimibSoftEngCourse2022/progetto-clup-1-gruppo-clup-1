...
class TestBasicQueueProvider(unittest.TestCase):
    ...
    def test_unexistent_store_id_gets_empty_queue_and_pool(self):
        ...
        pool = self.bqp.get_aisle_pool(store_id)
    ...
    def test_modifications_on_store_queues_are_presisted(self):
        ...
        self.assertTrue('b' in self.bqp.get_aisle_pool(store_id))
        self.assertEqual(self.bqp.get_aisle_pool(store_id).capacity, 10)
    ...
    def test_modifications_on_different_stores_act_on_different_queues(self):
        ...
        pool1 = self.bqp.get_aisle_pool(store1_id)
        queue2 = self.bqp.get_waiting_queue(store2_id)
        pool2 = self.bqp.get_aisle_pool(store2_id)
        ...
        self.assertTrue('c' not in self.bqp.get_aisle_pool(store2_id))
        self.assertTrue('d' not in self.bqp.get_aisle_pool(store1_id))
